...
Updated : 2025-07-03 (final zero-error build)
"""
...
def train_classifiers(X, y, pre) -> Dict[str, Dict]:
    ...
    for name, model in models.items():
        ...
        results[name] = {
            "pipe" : pipe,
            "train": {m: f(y_tr, pipe.predict(X_tr), pos_label="Yes")
                      for m, f in [("accuracy",  accuracy_score),
                                   ("precision", precision_score),
                                   ("recall",    recall_score),
                                   ("f1",        f1_score)]},
            "test" : {m: f(y_te, pipe.predict(X_te), pos_label="Yes")
                      for m, f in [("accuracy",  accuracy_score),
                                   ("precision", precision_score),
                                   ("recall",    recall_score),
                                   ("f1",        f1_score)]},
...
def tab_classification(df: pd.DataFrame):
    ...
    # Confusion matrix
    model_sel = st.selectbox("Model for confusion matrix", list(res))
    y_true    = res[model_sel]["y_test"]
    y_pred    = res[model_sel]["pipe"].predict(X.iloc[y_true.index])
    cm        = confusion_matrix(y_true, y_pred, labels=["No","Yes"])
    st.plotly_chart(px.imshow(cm, text_auto=True, x=["No","Yes"], y=["No","Yes"],
...
def tab_clustering(df: pd.DataFrame):
    ...
    st.plotly_chart(px.line(x=list(range(1,11)), y=inertias, markers=True,
...
def tab_regression(df: pd.DataFrame):
    ...
    target = st.selectbox("Numeric target variable",
    ...
    for n,m in models.items():
        ...
        if n in ("Linear","Ridge","Lasso"):
            ...
            with st.expander(f"{n} – Top coefficients"):
                st.plotly_chart(px.bar(coefs, x="coef", y="feature",
                                       orientation="h"),
                                use_container_width=True)
    ...
    resid  = y - preds
    st.plotly_chart(px.scatter(x=preds, y=resid,
                               labels={"x":"Predicted","y":"Residual"},
                               title="Residuals – Decision Tree"),
                    use_container_width=True)

# ─────────────── Tab 6 – 12-Month Retention ────────────────
def tab_retention(df: pd.DataFrame):
    ...
    try:
        ...
        st.warning(f"SHAP failed ({e}). Falling back to model-based importances.")
        imp = pd.DataFrame({
            "feature"   : pipe["prep"].get_feature_names_out(),
            "importance": getattr(mdl,"feature_importances_",
                                  np.zeros(X_trans.shape[1]))
        }).sort_values("importance", ascending=False).head(15)
